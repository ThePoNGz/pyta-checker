...
| The failure guard above decided under the scheduler's lock and published after releasing it. A `did_close` landing in that gap cleared the document and cancelled its check, and the failure diagnostic was then published onto a file no longer open, with nothing left to clear it. `fail()` takes the action and runs it inside the same critical section as the decision, the way `guard()` already does; `server.fail` passes its publish-and-notify closure. | `scheduler.fail`, `server.fail` | `test_fail_runs_its_action_before_another_thread_can_cancel`, `test_fail_completes_only_the_newest_generation` (`test_scheduler.py`); `test_a_failure_is_not_published_onto_a_document_that_was_just_closed` (`test_server.py`) |
| The `**kwargs` half of the row above went too far: `**kwargs` takes no positional slot, so `check_all('a1.py', {...}, **EXTRA)` carries its config in `args[1]` exactly as it did before that change, and treating the unpacking as if it hid the positions made the call come back `absent` and checked the file against stock defaults — a regression introduced in this same section. Only `*args` blocks a positional read now, and the `**kwargs` warning is no longer tied to a config that could not be found: it fires whenever one is present, since it can still carry a `config` or a `load_default_config` that cannot be seen. | `config_extract._argument`, `config_extract.extract_config` | `test_a_positional_config_is_still_read_beside_a_starred_keyword_dict`, `test_a_keyword_config_is_read_beside_a_starred_keyword_dict`, and the two starred tests above (`test_config_extract.py`) |
| `shutil.copyfile` writes into the destination before it raises, so the config copy that the row above made survivable left a *truncated* `config/.pylintrc` in staging when it died partway — a full disk, a read that failed mid-file — and `find_local_config` then handed that to python_ta. A half-read config is a worse answer than the defaults the warning already promises, so the destination is removed, errors ignored, before the warning is logged. | `server._check` | `test_a_config_copy_that_dies_midway_stages_nothing` (`test_server.py`) |
| Only the server's spawn env pinned `MYPY_CACHE_DIR`, so a check run in this process — the tests, or `python -m pyta_lsp.runner` by hand — let mypy write a `.mypy_cache` into whatever directory it was standing in. With the chdir rule from the row above, one landed in the repository root and was packaged into the VSIX. The per-user cache path moves to a module both sides share, and `run_check` sets it on `os.environ` if nothing else has. | `paths.mypy_cache_dir`, `runner.run_check`, `scheduler.runner_env` | `test_an_in_process_check_leaves_no_mypy_cache_in_the_cwd` (`test_runner.py`); the two `mypy_cache_dir` tests in `test_scheduler.py` still hold |
...
Where it ended: 157 Python tests and 75 TypeScript tests, green. Four of those rows were added after a review of the round itself; three of them undo or narrow something the round had just put in.
