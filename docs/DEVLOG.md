...
A second sweep over the server alone, after the release review in section 14. Eleven findings, all reviewed and all but one reproduced before being touched; the exception is noted below. Each was written as a failing test first and watched fail, then fixed, then the whole suite re-run. Going in: 113 Python tests.
...
**A residual on 3.10, deliberately.** `PYTHONSAFEPATH` is 3.11+. On 3.10 the only defence left is the directory the runner is spawned in, and the server now picks the directory of the file actually being checked: for a staged check that is a fresh `mkdtemp` holding the copy and, when there is one, a `config/` — nothing importable. For a package module, which has to be checked in place, it is the package directory, so on 3.10 a `string.py` sitting inside the student's own package can still be imported at runner startup. `strip_cwd_from_path()` stays as the fallback for everything after `main()` begins. Narrowing this further would mean not checking package modules in place, which section 13 already ruled out for a worse reason.
