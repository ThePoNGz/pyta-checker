...
- Dependency wheels: every dependency has a pure-Python wheel (`py3-none-any`) except aiohttp and markupsafe. aiohttp supports a pure build via `AIOHTTP_NO_EXTENSIONS=1`; markupsafe's C speedup is optional and the package falls back to pure Python when the compiled module is absent.
