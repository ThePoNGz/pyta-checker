# Third-party notices

PythonTA Checker bundles the following unmodified Python packages inside the extension (`bundled/libs/`). Each package's license text ships in its `*.dist-info` directory in the bundle where the package provides one. The extension's own code is MIT licensed (see `LICENSE`).

Generated by `scripts/bundle.py`; do not edit by hand.

| Package | Version | License | Homepage |
| --- | --- | --- | --- |
| aiohappyeyeballs | 2.7.1 | PSF-2.0 | https://github.com/aio-libs/aiohappyeyeballs |
| aiohttp | 3.14.3 | Apache-2.0 AND MIT | https://github.com/aio-libs/aiohttp |
| aiosignal | 1.4.0 | Apache 2.0 | https://github.com/aio-libs/aiosignal |
| astroid | 4.0.4 | LGPL-2.1-or-later | https://github.com/pylint-dev/astroid |
| async-timeout | 5.0.1 | Apache 2 | https://github.com/aio-libs/async-timeout |
| attrs | 26.1.0 | MIT | https://github.com/python-attrs/attrs |
| black | 26.5.1 | MIT | https://github.com/psf/black |
| cattrs | 26.2.0 | MIT | https://catt.rs |
| certifi | 2026.7.22 | MPL-2.0 | https://github.com/certifi/python-certifi |
| charset-normalizer | 3.5.1 | MIT | https://github.com/jawah/charset_normalizer |
| click | 8.5.0 | BSD-3-Clause | https://github.com/pallets/click/ |
| colorama | 0.4.6 | BSD License | https://github.com/tartley/colorama |
| dill | 0.4.1 | BSD-3-Clause | https://github.com/uqfoundation/dill |
| exceptiongroup | 1.3.1 | MIT License | https://github.com/agronholm/exceptiongroup |
| frozenlist | 1.8.0 | Apache-2.0 | https://github.com/aio-libs/frozenlist |
| idna | 3.20 | BSD-3-Clause | https://github.com/kjd/idna |
| isort | 9.0.1 | MIT | https://isort.readthedocs.io/ |
| Jinja2 | 3.1.6 | BSD License | https://github.com/pallets/jinja/ |
| lsprotocol | 2025.0.0 | MIT License | https://github.com/microsoft/lsprotocol |
| markdown-it-py | 4.2.0 | MIT License | https://github.com/executablebooks/markdown-it-py |
| MarkupSafe | 3.0.3 | BSD-3-Clause | https://github.com/pallets/markupsafe/ |
| mccabe | 0.7.0 | Expat license | https://github.com/pycqa/mccabe |
| mdurl | 0.1.2 | MIT License | https://github.com/executablebooks/mdurl |
| multidict | 6.9.1 | Apache License 2.0 | https://github.com/aio-libs/multidict |
| mypy | 1.18.2 | MIT | https://www.mypy-lang.org/ |
| mypy_extensions | 1.1.0 | MIT | https://github.com/python/mypy_extensions |
| packaging | 26.3 | Apache-2.0 OR BSD-2-Clause | https://github.com/pypa/packaging |
| pathspec | 1.1.1 | Mozilla Public License 2.0 (MPL 2.0) | https://github.com/cpburnz/python-pathspec |
| platformdirs | 4.11.12 | MIT | https://github.com/tox-dev/platformdirs |
| propcache | 0.5.4 | Apache-2.0 | https://github.com/aio-libs/propcache |
| pycodestyle | 2.14.0 | MIT | https://pycodestyle.pycqa.org/ |
| pygls | 2.1.1 | Apache-2.0 |  |
| Pygments | 2.20.0 | BSD-2-Clause | https://pygments.org |
| pylint | 4.0.8 | GPL-2.0-or-later | https://github.com/pylint-dev/pylint |
| python-ta | 2.13.1 | MIT | https://github.com/pyta-uoft/pyta |
| pytokens | 0.4.1 | MIT License | https://github.com/tusharsadhwani/pytokens |
| requests | 2.34.2 | Apache-2.0 | https://github.com/psf/requests |
| six | 1.17.0 | MIT | https://github.com/benjaminp/six |
| tabulate | 0.10.0 | MIT | https://github.com/astanin/python-tabulate |
| toml | 0.10.2 | MIT | https://github.com/uiri/toml |
| tomli | 2.4.1 | MIT | https://github.com/hukkin/tomli |
| tomlkit | 0.15.1 | MIT | https://github.com/python-poetry/tomlkit |
| typeguard | 4.6.0 | MIT | https://github.com/agronholm/typeguard |
| typing_extensions | 4.16.0 | PSF-2.0 | https://github.com/python/typing_extensions |
| urllib3 | 2.8.0 | MIT | https://github.com/urllib3/urllib3 |
| wrapt | 2.4.1 | BSD-2-Clause | https://github.com/GrahamDumpleton/wrapt |
| yarl | 1.25.1 | Apache-2.0 | https://github.com/aio-libs/yarl |

Note: python-ta's wheel ships no license file; its metadata declares MIT while the repository's LICENSE file is GPL-3.0. This project redistributes the wheel as published. See https://github.com/pyta-uoft/pyta.
