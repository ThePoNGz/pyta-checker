...
The checker is a standalone language server speaking LSP over stdio, so any editor that speaks LSP can use it. Releases after v0.1.0 attach `pyta-lsp-server-<version>.tar.gz`: extract it, put its `libs` directory on `PYTHONPATH`, and start it with Python 3.10 or newer as `python -c` with the `SERVER_LAUNCHER` line in `scripts/bundle.py`, which is what the Zed extension does. The line contains backslash sequences, so in a JSON editor config each backslash is doubled, the way `zed/src/logic.rs` writes it. Started that way the server is safe from any directory. If the directory it starts in is also on `PYTHONPATH`, checks still import packages from it, but only after the standard library. Keep any other directory holding student files off `PYTHONPATH`, since a `types.py` there is imported before the server can guard against it. `python -m pyta_lsp` also works, but only from a directory holding no student files and with a `PYTHONPATH` holding nothing but `libs`. Until then, `python scripts/bundle.py build` in a checkout produces the same `libs` directory as `bundled/libs`. If you want a specific editor supported, open a GitHub issue.
